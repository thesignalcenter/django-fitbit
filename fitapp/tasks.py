...
def get_intraday_data(fitbit_user, category, resource, date, tz_offset):
    """Get the given Fitbit user's intraday data for a specified date and save it in the database.

    Convert timestamps to UTC prior to saving.

    The Fitbit API stipulates that intraday data can only be retrieved for one
    day at a time.

    Args:
        fitbit_user (str): The Fitbit user ID from which to retrieve data.
            Can come from UserFitbit.fitbit_user.
        category (enum): One of the enumerated choices for TimeSeriesDataType.category.
        resource (str): The resource as defined by the Fitbit API that we want to retrieve data for.
        date (date): The day for which to retreive intraday data.
        tz_offset (int): Offset in hours of the user's timezone to UTC.
    """

    # Ensure that the given category and resource are valid.
    try:
        time_series_data_type = TimeSeriesDataType.objects.get(category=category, resource=resource)
    except TimeSeriesDataType.DoesNotExist:
        logger.exception("The resource %s in category %s doesn't exist" %
                         (resource, category))
        raise Reject(sys.exc_info()[1], requeue=False)

    # Ensure that the requested TimeSeriesDataType supports intraday data.
    if not time_series_data_type.intraday_support:
        logger.exception("The resource %s in category %s does not support "
                         "intraday time series" % (resource, category))
        raise Reject(sys.exc_info()[1], requeue=False)
        ...
        with transaction.atomic():
            for fbuser in fbusers:
                ...
                for minute in intraday:
                    ...
                    tsd, created = TimeSeriesData.objects.get_or_create(
                        user=fbuser.user, resource_type=time_series_data_type, date=utc_datetime,
                        intraday=True)
                    tsd.value = value
                    tsd.save()
            # Release the lock
